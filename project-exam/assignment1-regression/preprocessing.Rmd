...
rf <- randomForest(int_rate ~ ., data = sampled_train_data, ntree = 500)
...
# Make predictions on the training and testing data
rf.train_predictions <- predict(rf, newdata = sampled_train_data)
rf.test_predictions <- predict(rf, newdata = sampled_test_data)

# Calculate Mean Squared Error (MSE) for training and testing
rf.train_mse <- mean((rf.train_predictions - sampled_train_data$int_rate)^2)
rf.test_mse <- mean((rf.test_predictions - sampled_test_data$int_rate)^2)
...
# Calculate Mean Absolute Error (MAE) for training and testing
rf.train_mae <- mean(abs(rf.train_predictions - sampled_train_data$int_rate))
rf.test_mae <- mean(abs(rf.test_predictions - sampled_test_data$int_rate))

# Calculate R-squared (R²) for training and testing
rf.train_r2 <- 1 - (sum((sampled_train_data$int_rate - rf.train_predictions)^2) / sum((sampled_train_data$int_rate - mean(sampled_train_data$int_rate))^2))
rf.test_r2 <- 1 - (sum((sampled_test_data$int_rate - rf.test_predictions)^2) / sum((sampled_test_data$int_rate - mean(sampled_test_data$int_rate))^2))
...
```
Following, a scatter plot of actual vs predicted values for each model is plot.
...
create_scatter_plot <- function(actual_values, predicted_values, model_name) {
  ...
  scatter_plot <- ggplot(model_comparison_data, aes(x = Actual, y = Predicted)) +
    ...
    labs(x = "Actual Values", y = "Predicted Values", title = model_name) +
    theme_minimal()
...
rf_scatter_plot <- create_scatter_plot(
  actual_values = sampled_train_data$int_rate,
  predicted_values = rf.train_predictions,
