...
# read the csv
lcdata <- read.csv("LCdata.csv", sep = ";", fileEncoding="UTF-8")
print(names(data))
lcdata <- subset(lcdata, select = -c(collection_recovery_fee, installment, issue_d,
...
cat_col_names <- names(lcdata)[sapply(lcdata, is.character)]

# One-Hot encode all categorical columns
lc_data <- dummy_cols(lcdata, select_columns = cat_col_names, remove_selected_columns = TRUE)
